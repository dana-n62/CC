...
### Manual Testing
```bash
# Valid map - should play successfully
./so_long maps/valid_simple_map.ber

# Invalid maps - should fail with appropriate errors
./so_long maps/invalid_rect.ber
...
### Memory Check
```bash
valgrind --leak-check=full --show-leak-kinds=all ./so_long maps/valid_simple_map.ber
```
...
**Expected Result:** No errors (all files OK)
